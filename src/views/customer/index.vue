  ...
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="操作" width="200px" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button type="danger" icon="el-icon-delete" circle @click="handleCustomerDelete(scope.row)" />
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :visible.sync="userEditDialogVisible" title="用户编辑">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="handleCustomerUpdate">确定</el-button>
      </div>
    </el-dialog>
    <!-- 用户添加对话框 -->
    <el-dialog :visible.sync="userAddDialogVisible" title="用户添加">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="handleCustomerAdd">确定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
<script>
import { getCustomers, updateCustomer, addCustomer, deleteCustomer, searchCustomer } from '@/api/customer'
...
export default {
  ...
  methods: {
    ...
    handleCustomerUpdate() {
      updateCustomer(this.userDetail)
        .then(() => {
    ...
    handleCustomerDelete(row) {
      this.userDetail = JSON.parse(JSON.stringify(row))
      deleteCustomer(this.userDetail)
        .then(() => {
          this.userDialogVisible = false
          this.$notify.success({
            title: '成功',
    ...
    handleSearch() {
      ...
      searchCustomer({ id: this.filterText }).then(response => {
