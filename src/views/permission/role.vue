...
<script>
import { getRoles, addRole, updateRole, deleteRole, getPermission, updatePermission } from '@/api/role'
  ...
  methods: {
    ...
    handlePermission(row) {
      ...
      getPermission({ roleId: row.id })
        .then(response => {
          this.systemPermissions = response.data[0].systemPermissions
          this.assignedPermissions = response.data[0].assignedPermissions
        })
    },
    updatePermission() {
